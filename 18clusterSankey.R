...
clinical = clinicalRT[,c('Sample','Event','ATA','TNM','HistologicalType'),drop = F]

merge2 = merge(score, clinical, by.x = 'id', by.y = "Sample")   
merge3 = merge(clusterRT,merge2, by.x = 'ID', by.y = "id")

data = merge3[,c(2:7),drop=F]
data = na.omit(data)

data$TNM = ifelse(data$TNM =='Stage I','Stage I',
                       ifelse(data$TNM =='Stage II','Stage II','Stage III&IV'))
data$Cluster = ifelse(data$Cluster == 'C1','C1',
                      ifelse(data$Cluster == 'C2','C2','C3'))
data$HistologicalType=ifelse(data$HistologicalType=='Classical','Classical',
                             ifelse(data$HistologicalType=='Follicular variant','Follicular','Aggressive'))
data$Event=ifelse(data$Event=='0','Censored','Event')
...
colnames(data)[colnames(data)=="Event"] <- "TCFi"  
colnames(data)[colnames(data)=="HistologicalType"] <- "Type"  


corLodes = to_lodes_form(data, axes = 1:ncol(data), id = "Cohort")

corLodes$stratum = factor(corLodes$stratum, 
                          levels = c('C1','C2','C3','High-risk','Low-risk','Event','Censored','High','Intermediate','Low',#RR RS NR
                                     'Stage III&IV','Stage II','Stage I','Aggressive','Classical','Follicular'))
...
ggsave('Sankey.pdf', height = 3.5, width = 5)
